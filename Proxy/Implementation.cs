...
namespace Proxy
{
    ...
    public class ProtectedDocumentProxy : IDocument
    {
        ...
        private DocumentProxy _documnetProxy;
        public ProtectedDocumentProxy(string fileName,string userRole)
        {
            _fileName = fileName;
            _userRole = userRole;
            _documnetProxy= new DocumentProxy(_fileName);
        }
        public void DisplayDocument()
        {
            Console.WriteLine($"Entering DispalayDocument"+
                $"in {nameof(ProtectedDocumentProxy)}.");
            if (_userRole != "Viewer" )
            {
                throw new UnauthorizedAccessException();    
            }
            _documnetProxy.DisplayDocument();

            Console.WriteLine($"Existing DispalayDocument" +
              $"in {nameof(ProtectedDocumentProxy)}.");
        }
    }
}
